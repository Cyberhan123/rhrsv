...
function popStatus(status, html, timeout, url, bremove) {
	...
	$('body #bremove').remove();
	if (status == 1) {
		$('body').append('<div id="wstatus"><div class="wstatus_s wstatus_s1"></div><span class="wstatus_f">'
		+html+'</span></div>');
	}else if (status == 2) {
		$('body').append('<div id="wstatus"><div class="wstatus_s wstatus_s2"></div><span class="wstatus_f">'
		+html+'</span></div>');
	}else if (status == 3) {
		$('body').append('<div id="wstatus"><div class="wstatus_s wstatus_s3"></div><span class="wstatus_f">'
		+html+'</span></div>');
	}else {
		$('body').append('<div id="wstatus"><div class="wstatus_s wstatus_s4"></div><span class="wstatus_f">'
		+html+'</span></div>');
	}
	popStatuRe();
	//是否使用遮照
